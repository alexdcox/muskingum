...
import {DoubledCoord, Hex} from "./hex.js"
...
export class GameServer {
  ...
  public games: Game[] = []
  public registeredPlayers: Player[] = []
  public combatantQueue: Player[] = []
  ...
  handleConnectionClosed(socket: WebSocket) {
  ...
  handleMessage(message: any, socket: WebSocket) {
    ...
    switch(message.type) {
      case 'summon':
        const unitId: UnitId = message.unitid
  ...
  startGame() {
    ...
    game.eventEmitter.on('gamestate', (game: Game) => {
    ...
    game.eventEmitter.on('gamestats', (game: Game) => {
...
export class Game {
  ...
  constructor(player1: Player, player2: Player) {
    ...
    const summoner: Unit = UnitId.unit(UnitId.Summoner)
  ...
  summon(player: Player, unitId: UnitId, coord: DoubledCoord) {
    const playerState = this.playerState[this.turn.playerIndex]
  ...
  move(player: Player, from: DoubledCoord, to: DoubledCoord) {
    if (player.id != this.players[this.turn.playerIndex].id) {
  ...
  attack(player: Player, from: DoubledCoord, to: DoubledCoord) {
    if (player.id != this.players[this.turn.playerIndex].id) {
  ...
  recordDrawnUnitCost(player: Player, cost: number) {
...
class GamePlayerState {
  ...
  public energyGained: number = 0
  public energySpent: number = 0
  public unitsSummoned: number = 0
  public unitsLost: number = 0
...
export interface GameStats {
  ...
  player: {
    ...
    energyGained: number
    energySpent: number
    // drawCostDistribution = unitCost:numberOfTimesDrawn
